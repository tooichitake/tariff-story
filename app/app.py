...
with st.sidebar:
    ...
    # for motion's sake. The Act IV countdown is computed live from DC
    # time so the sidebar stays in lockstep with the Hook and Act IV
    # countdown widgets (all three would otherwise drift by a day each
    # midnight in Washington).
    # Date-only subtraction (matches Act IV's `date - date` formula). Using
    # `datetime - datetime` would subtract time-of-day too, making `.days`
    # truncate to one day short of Act IV whenever DC isn't exactly at
    # midnight — which is always.
    _ET = timezone(timedelta(hours=-4))
    _sb_days_left = (date(2026, 7, 24) - datetime.now(_ET).date()).days
    st.markdown(f"""
    ...
    <nav class="sb-nav">
      ...
      <a href="#act-iv">
        ...
        <span><span class="sb-title">The Choice</span><br>
              <span class="sb-question">{_sb_days_left} days to decide.</span></span>
      </a>
    </nav>
    """, unsafe_allow_html=True)
...
# Act II closing transition — derive the two rhetorical anchors from the
# same viz3 Current Policy rows that power the Hook + Act II dumbbell,
# so all three read from one source of truth. No TPC-vs-Yale drift.
# Burden rounded to nearest $10, income to nearest $1k — false precision
# like "$37,601" hurts the cadence.
...
_d1_income = round(_d1_cost_raw / (abs(float(_d1["pct_income_lost"])) / 100.0), -3)
_d10_income = round(_d10_cost_raw / (abs(float(_d10["pct_income_lost"])) / 100.0), -3)
st.markdown(transition_text(
    f'"${_d1_cost:,.0f} to a family earning ${_d1_income:,.0f} is two weeks of rent.<br>'
    f'${_d10_cost:,.0f} to a family earning ${_d10_income:,.0f} is a restaurant tab.<br>'
