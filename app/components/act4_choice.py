...
Scenario × Decile decision matrix (heatmap of 5 scenarios × 10 income slices)
What-If slider: interactively set tariff rate, see all 10 decile burdens
...
def render(viz7, viz8, viz3, customs, selected_event, selected_event_row=None):
    ...
    Visuals:
      ...
      - Scenario × Decile decision matrix (5 policy scenarios × 10 income
        slices, heat-mapped by USD burden)
      - What-If slider — interactively set tariff rate, see all 10 decile
    ...
    Args:
        viz7: DataFrame — 5 policy scenarios with macro params. Required cols:
            scenario, eff_tariff_rate (used to build the heatmap scale).
            Other columns (household_cost_bottom20_usd, gdp_impact_pct, etc.)
            may be present but are no longer read by this render function.
    ...
    st.markdown(chart_header(
        ...
        "Annual tariff cost (USD) for each household decile under each policy "
        "scenario. Source: Yale Budget Lab distributional analysis (Feb 2026); "
        "non-endpoint scenarios are interpolated between Current Policy (13%) "
        "and IEEPA Upheld (27%) tariff rates."
    ...
    if (
        ...
        # Reader-facing column labels: typical household income per decile,
        # not D1/D10 (data-science jargon). Matches the Act II pyramid and
        # bar chart so the reader reads the same names across acts.
        decile_labels = list(DECILE_INCOME_LABELS)
        ...
        fig_matrix = go.Figure(data=go.Heatmap(
            ...
            hovertemplate=(
                "<b>%{y}</b><br>Household earning %{x}<br>"
                "Annual cost: %{text}<extra></extra>"
            ),
            showscale=False,
        ))
        fig_matrix.update_layout(
            **CHART_LAYOUT,
            xaxis=dict(title_text="Typical household income (poorest → richest)",
    ...
    st.markdown(chart_header(
        ...
        "Adjust the rate to any value between 0% and 25%. Every setting taxes "
        "the poorest at a higher share. Source: Yale Budget Lab distributional "
        "analysis (Feb 2026); decile burdens interpolated between Current Policy "
        "and IEEPA Upheld scenarios."
    ), unsafe_allow_html=True)
    ...
    if len(current_scenario) == 10 and len(ieepa_scenario) == 10:
        ...
        fig_whatif = go.Figure(go.Bar(
            y=list(DECILE_INCOME_LABELS),
            x=interp_pct,
            orientation="h",
            marker=dict(color=DECILE_COLORS),
            text=[f"{v:.2f}%" for v in interp_pct],
            textposition="outside",
            hovertemplate="<b>Household earning %{y}</b><br>Income Lost: %{x:.2f}%<extra></extra>",
        ))
