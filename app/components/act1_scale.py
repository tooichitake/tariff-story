...
def render(viz1, key_events, viz6_anim, selected_event, selected_event_row=None):
    ...
    Narrative intent:
        ...
        Peak-rate metric + days-to-peak climb + animated world map sell the
        scale. The days-to-peak number is derived live from viz1 rather than
        hardcoded, so a data refresh that shifts the peak automatically
        updates both the metric card and the chart title.
    """
    ...
    _peak_date = pd.to_datetime(viz1.loc[viz1["eff_tariff_rate"].idxmax(), "date"]).date()
    _days_to_peak = (_peak_date - _date(2025, 1, 20)).days
    _peak_window = f"Jan 20 - {_peak_date.strftime('%b')} {_peak_date.day}"
    ...
    with col2:
        with styled_metric_card(COLORS["gold"]):
            st.metric("Time to Peak", f"{_days_to_peak} days", _peak_window)
    with col3:
    ...
    st.markdown(chart_header(
        f"Century-High Tariffs in {_days_to_peak} Days",
